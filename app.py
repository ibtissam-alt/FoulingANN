...
import os, pandas as pd, sys, tensorflow
...
class Main(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.setWindowTitle("Fouling ANN PFE")
    ...
    def ANN(self):
        ...
        self.textBrowser_2.setText(
            f"Mean absolute error (MAE):     {round(sklearn.metrics.mean_absolute_error(self.y_test, self.y_pred), 5)}" )
        self.textBrowser_2.append(
            f"Mean squared error (MSE):     {round(sklearn.metrics.mean_squared_error(self.y_test, self.y_pred), 5)}")
        self.textBrowser_2.append(f"Root mean squared error (RMSE): {round(math.sqrt( sklearn.metrics.mean_squared_error(self.y_test, self.y_pred)), 5)}")
        self.textBrowser_2.append(
            f"R square (R^2):                 {round(sklearn.metrics.r2_score(self.y_test, self.y_pred), 5)}")
    ...
    def loadCsv(self):
        ...
        if self.fileName and os.path.isfile(self.fileName):
            ...
            if len(columns) == 8:
                ...
                self.push.setText(f"ERROR : {len(columns)} columns found, 8 expected")
